    ...
    if logos_pos in one_codes:
        ...
        if return_codes["mood"] == "N": # infinitive
            ...
            if morph[6] in case_map:
                ...
                print(f"Unknown case: {morph[6]} from {morph}")
            if morph[5] in number_map:
                return_codes["number"] = number_map[morph[5]]
            else:
                print(f"Unknown number: {morph[5]} from {morph}")
            if morph[7] in gender_map:
                return_codes["gender"] = gender_map[morph[7]]
            else:
                print(f"Unknown gender: {morph[7]} from {morph}")
            return format_return_codes(return_codes)
        else:
            return_codes["person"] = morph[4]
            return_codes["number"] = number_map[morph[5]]
